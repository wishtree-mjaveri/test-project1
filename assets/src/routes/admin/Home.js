...
import {
  Button, Card, Divider, Table, Tooltip, Layout, message, Pagination, PaginationProps, TablePaginationConfig, Popconfirm, Spin,
} from 'antd';
import Icon, {
  EditTwoTone, DeleteTwoTone, ArrowDownOutlined, ArrowUpOutlined,
  ...
  const columns = [
    {
      title: <span>
        {' '}
        <IntlMessages id="Adminhome.table.restaurantname" />
        {' '}
        <Tooltip title="Ascending Order" placement="topLeft">
          {' '}
          <ArrowUpOutlined onClick={() => setSortorder('ASC')} />
        </Tooltip>
        {' '}
        <Tooltip title="Descending Order" placement="topLeft">
          {' '}
  ...
  const handleLogout = () => {
    Axios.post(
      ...
      .then((res) => {
        ...
        localforage.setItem('user-key','',(err,val)=>{
          if (err) {
            console.log(err)
          }
          console.log(val)
